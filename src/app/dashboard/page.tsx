...
type TabState = "main" | "nguontien" | "ngansach" | "khoanno";
    ...
    <div className="min-h-screen bg-slate-950 flex justify-center font-sans text-slate-200">
      <div className="w-full max-w-md bg-slate-900 relative flex flex-col shadow-2xl h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto pb-28 relative">
          ...
          {/* NẾU ĐANG Ở TRANG CHỦ (MAIN) - Đã dọn dẹp các khung không cần thiết */}
          {activeTab === "main" && (
            <div className="animate-in fade-in duration-500">
              ...
              {/* Không gian trống chờ tính năng Tổng quan sau này */}
        ...
        {/* === THANH ĐIỀU HƯỚNG BÊN DƯỚI (UI ĐÃ ĐƯỢC LÀM MƯỢT) === */}
        <div className="absolute bottom-0 w-full bg-slate-900/95 backdrop-blur-md border-t border-emerald-500/20 px-2 py-3 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.6)] z-40 rounded-t-3xl">
          ...
          <div className="flex flex-1 justify-evenly items-center">
            {/* Nút Tổng Quan */}
            <button 
              onClick={() => showGameFiAlert('Tổng quan')} 
              className="group flex flex-col items-center gap-1.5 p-2 rounded-2xl opacity-60 hover:opacity-100 hover:bg-slate-800/60 active:scale-90 transition-all duration-300 ease-in-out"
            >
              <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center shadow-inner group-hover:bg-slate-700 transition-colors duration-300">
                <span className="text-sm">👁️</span>
              </div>
              <span className="text-[10px] font-bold text-slate-400 tracking-wide">Tổng quan</span>
            </button>
          ...
          {/* Nút Trung Tâm (Thêm Mới) */}
          <div className="relative -top-8 flex-shrink-0 px-2">
            <button 
              onClick={() => showGameFiAlert('Thêm Giao Dịch')} 
              className="group w-16 h-16 bg-gradient-to-tr from-emerald-600 to-emerald-400 rounded-full flex items-center justify-center border-4 border-slate-900 shadow-[0_0_20px_rgba(16,185,129,0.5)] hover:shadow-[0_0_25px_rgba(16,185,129,0.7)] active:scale-90 transition-all duration-300"
            >
              <span className="text-4xl text-slate-900 font-black leading-none pb-1 group-hover:rotate-90 transition-transform duration-300 ease-bounce">＋</span>
            </button>
          </div>
