...
export default function NguonTien() {
  ...
  // State mới: Quản lý Số thứ tự nhập vào
  ...
  // Tự động mớm STT tiếp theo vào ô nhập mỗi khi danh sách thay đổi
  useEffect(() => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Database dùng Index bắt đầu từ 0, còn UI hiển thị từ 1 nên phải trừ đi 1
  ...
  return (
    <div className="w-full h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-300 relative">
      ...
      <div className="p-3 space-y-4 overflow-y-auto pb-24 flex-1">
        ...
        {/* KHUNG TẠO NGUỒN MỚI (Đã bổ sung ô nhập STT) */}
        <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-3 shadow-sm">
          <form onSubmit={handleSave} className="flex gap-2 items-center">
            ...
            <input 
              type="number" 
              value={thuTuInput}
              onChange={(e) => setThuTuInput(e.target.value ? Number(e.target.value) : "")}
              placeholder="STT" 
                    ...
                    <div className="flex items-stretch w-full">
                      ...
                      <div className="flex flex-col items-center justify-center bg-slate-800 border-r border-slate-700/50 w-10 shrink-0">
                        <button 
                          ...
                          disabled={index === 0}
                          className="flex-1 w-full flex items-center justify-center text-slate-500 hover:text-emerald-400 hover:bg-slate-700 active:bg-slate-600 disabled:opacity-20 transition-all"
                        >
                          ▲
                        </button>
                        ...
                        <button 
                          ...
                          className="flex-1 w-full flex items-center justify-center text-slate-500 hover:text-orange-400 hover:bg-slate-700 active:bg-slate-600 disabled:opacity-20 transition-all"
                        >
                          ▼
                        </button>
                      </div>

                      {/* NỘI DUNG CHÍNH (BẤM VÀO ĐỂ MỞ CHI TIẾT) */}
                      <div 
                        onClick={() => handleToggleExpand(nguon.nguon_tien_id)}
                        className="flex-1 flex items-center justify-between p-3 cursor-pointer hover:bg-slate-800/50 transition-colors"
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-slate-600 w-4">{index + 1}.</span>
                          <div className="flex flex-col ml-1">
                            <span className="font-black text-blue-400 text-sm uppercase tracking-tight">{nguon.ten_nguon}</span>
