...
import { GetChartNganSachUseCase } from "@/2_use_cases/statistics/GetChartNganSachUseCase";
...
export default function ChartNganSach({ onClose }: ChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const useCase = new GetChartNganSachUseCase();
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-2">
      <div className="w-full max-w-5xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col h-[calc(100dvh-1rem)] sm:h-[600px] overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-900/50 shrink-0">
          <h2 className="text-orange-400 font-black uppercase tracking-widest text-xs sm:text-sm">Phân Tích Tiêu Dùng Ngân Sách</h2>
        ...
        <div className="flex-1 flex flex-col p-2 sm:p-4 overflow-hidden min-h-0">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="flex bg-slate-950/50 p-1 rounded-lg border border-slate-800 shrink-0">
                {(['tuan', 'thang', 'nam'] as const).map(t => (
                  <button key={t} onClick={() => setGroupBy(t)} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${groupBy === t ? 'bg-orange-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
              ...
              <div className="flex items-center justify-between bg-slate-950/50 px-3 py-1.5 rounded-lg border border-slate-800 flex-1 text-xs">
                ...
                <select value={timeLimit} onChange={(e) => setTimeLimit(e.target.value === "ALL" ? "ALL" : Number(e.target.value))} className="bg-transparent text-orange-400 font-black focus:outline-none">
                  <option value={5} className="bg-slate-900">5 kỳ gần nhất</option>
                  <option value={10} className="bg-slate-900">10 kỳ gần nhất</option>
          ...
          <div className="flex-1 w-full bg-slate-950/50 rounded-xl border border-slate-800 p-2 overflow-x-auto custom-scrollbar">
            {isLoading ? <div className="h-full flex items-center justify-center text-slate-500 animate-pulse">Đang tải dữ liệu...</div> : (
